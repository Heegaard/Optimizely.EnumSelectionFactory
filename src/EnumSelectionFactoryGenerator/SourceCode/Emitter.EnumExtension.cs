...
internal sealed partial class Emitter
{
    internal IEnumerable<CodeSource> GetEnumExtensions(List<EnumTemplate> enums, string sharedNamespace)
    {
        foreach (var enumTemplate in enums)
        {
            ...
            string CreateEnumExtension() =>
                ...
                .Method($"public static string ToDisplayName(this {enumTemplate.FullName} value)", methodBuilder => methodBuilder
                    ...
                    .Line("{")
                    .Tab()
                    .Foreach(enumTemplate.Values.Where(x => x.Name is not null), (b, c) => b.Line($"{enumTemplate.FullName}.{c.Value} => \"{c.Name}\","))
                    .Line("_ => value.ToString()")
                    .Tab(-1)
                    .Line("};")
                    )
                .Method($"public static string ToDisplayShortName(this {enumTemplate.FullName} value)", methodBuilder => methodBuilder
                    .Tab()
                    .Line("return value switch")
                    .Line("{")
                    .Tab()
                    .Foreach(enumTemplate.Values.Where(x => x.ShortName is not null), (b, c) => b.Line($"{enumTemplate.FullName}.{c.Value} => \"{c.ShortName}\","))
                    .Line("_ => value.ToString()")
                    .Tab(-1)
                    .Line("};")
                    )
        ...
        string CreateSharedEnumExtension() =>
                ...
                .Method($"public static string ToDisplayShortName(this Enum value)", methodBuilder => methodBuilder
                    .Tab()
                    .Line("return value switch")
                    .Line("{")
                    .Tab()
                    .Foreach(enums, (b, c) => b.Line($"{c.FullName} propertyEnum => propertyEnum.ToDisplayShortName(),"))
                    .Line("_ => value.ToString()")
                    .Tab(-1)
                    .Line("};")
                    )
